...
class MultiScaleSeasonMixing(nn.Module):
    ...
    def forward(self, season_list):
        ...
        for i in range(len(season_list) - 1):
            # out_high = self.alpha *  self.MKAN_blocks[i](out_high.permute(0, 2, 1)).permute(0, 2, 1) + out_high
...
class PastDecomposableMixing(nn.Module):
        ...
        self.Conv_blocks = nn.ModuleList(
            [
                nn.Sequential(
                    ...
                    nn.GELU(),
                    nn.Conv1d(configs.d_model * 2, configs.d_model*2, kernel_size=3, padding=1),
                    nn.GELU(),
    ...
    def forward(self, x_list):
        ...
        for i in range(len(x_list)):
            ...
            # x_list[i] = self.Simam(x_list[i])
            # x_list[i] = x_list[i].squeeze(1)
            # x_list[i] = self.dyt(x_list[i])
            x_conv = self.Conv_blocks[i](x_list[i].permute(0, 2, 1)).permute(0, 2, 1)
            # x_kan = self.KAN_blocks[i](x_list[i].permute(0, 2, 1)).permute(0, 2, 1)
            x_list[i] = self.alpha *  x_conv \
                       + x_list[i]
